...
14. **The pause chord is an ordinary chord, never a media key** — `Cmd+Shift+P` by default, persisted as `pause_shortcut`. Every persisted shortcut passes `Settings::normalize_shortcut`'s media-key screen, and `apply_shortcut_with` screens again on the way to `register()`. This is the same "no `CGEventTap`, ever" rule as the region chord (hard constraint via `src/shortcut.rs`), and it is why pause keeps working while another app owns the media keys. `MPRemoteCommandCenter` is the *only* sanctioned route to the media key and it is a separate, still-ungated phase — see the research doc's §2 hand-back test, which has not been run.
